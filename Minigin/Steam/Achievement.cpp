...
// Global definitions (declared extern in Achievement.h)
...
void CSteamAchievements::OnUserStatsReceived(UserStatsReceived_t* pCallback)
{
	...
	if (k_EResultOK == pCallback->m_eResult)
	{
		...
		// Load achievement state and display info from Steam
	...
	{
		char buffer[128];
		_snprintf_s(buffer, sizeof(buffer), "RequestStats - failed, %d\n", pCallback->m_eResult);
...
void CSteamAchievements::OnUserStatsStored(UserStatsStored_t* pCallback)
{
	...
	{
		char buffer[128];
		_snprintf_s(buffer, sizeof(buffer), "StatsStored - failed, %d\n", pCallback->m_eResult);
		OutputDebugString(buffer);
	}
}
